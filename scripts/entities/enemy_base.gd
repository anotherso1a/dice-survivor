...
@export var debug: bool = false                                   # 调试开关：开启后打印状态变化日志
...
func _physics_process(_delta: float) -> void:
	...
	if has_node("StatusComponent"):
		var status = get_node("StatusComponent") as StatusComponent
		if status and status.has_status(&"freeze"):
			return  # 冰冻中，暂停移动
...
## 受到伤害（由玩家的骰子实体/子弹调用，传递给 HealthComponent）
## 调用方：player.gd（单目标/AOE/穿透子弹）、bullet.gd _handle_hit()
## dmg：伤害值 | is_crit：是否暴击 | face：骰面数据（FaceData 强类型）
func take_damage(dmg: int, is_crit: bool, face: FaceData) -> void:
	if _is_dying:
		return

	## 先弹出伤害数字，再扣血（敌人可能当场死亡，但数字已创建）
	_show_damage_number(global_position, dmg, is_crit)
...
func _show_damage_number(pos: Vector2, dmg: int, is_crit: bool) -> void:
	...
	lbl.modulate.a = 0.0
	lbl.z_index = 9999  # 显示在最上层

	## 挂到当前场景根节点
	var scene_root := get_tree().current_scene
	if scene_root != null:
		scene_root.add_child(lbl)
		## 用局部坐标（scene_root 的局部空间 = 全局空间，因为根节点位置在 (0,0)）
		lbl.position = pos + Vector2(randf_range(-20, 20), -30)
	else:
		add_child(lbl)
		lbl.position = Vector2(randf_range(-20, 20), -30)
...
func _die() -> void:
	...
	EventBus.enemy_died.emit(global_position, null)  # 发射 EventBus 信号，供击杀计数等系统使用
...
## 状态变化回调（由 StatusComponent.status_changed 信号触发）
func _on_status_changed(status_type: StringName, is_active: bool) -> void:
	if debug:
		print("[Enemy] 状态变化: %s | %s" % [status_type, "应用" if is_active else "移除"])


## 点燃伤害回调（由 StatusComponent.burn_tick 信号触发）
## 注意：简化版 - 直接调用 HealthComponent，未来应由 BurnComponent 统一处理
func _on_burn_tick(damage: int) -> void:
	if _health:
		_health.take_damage(damage, false)  # 点燃伤害不暴击，无 FaceData
	if debug:
		print("[Enemy] 点燃伤害: %d | HP: %d/%d" % [damage, _health.current_hp, _health.max_hp])
